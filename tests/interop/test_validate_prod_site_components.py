...
def test_validate_prod_site_reachable(kube_config, openshift_dyn_client):
    ...
    try:
        prod_api_url = application.get_site_api_url(kube_config)
        prod_api_response = application.get_site_api_response(
            openshift_dyn_client, prod_api_url, namespace, sub_string
        )
    except AssertionError as e:
        logger.error(f"FAIL: {e}")
        assert False, e

    if prod_api_response.status_code != 200:
        err_msg = "Prod site is not reachable. Please check the deployment."
...
def test_validate_argocd_applications_health_prod_site(openshift_dyn_client):
    ...
    if unhealthy_apps:
        err_msg = "Some or all applications deployed on prod site are unhealthy"
        logger.error(f"FAIL: {err_msg}:\n{unhealthy_apps}")
        assert False, err_msg
    else:
        logger.info("PASS: All applications deployed on prod site are healthy.")
